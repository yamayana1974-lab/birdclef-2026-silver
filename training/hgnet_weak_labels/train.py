...
或者:
    ...
    cd /path/to/birdclef-2026-repo/training/hgnet_weak_labels
    python train.py
...
from .config import CFG, N_CLASSES, N_FOLDS, PSEUDO_CSV, PSEUDO_SC_DIR, RANDOM_SEED, make_run_dir
from .data import (
...
from .pseudo_pool import PseudoPool, load_pseudo_pool
...
def train_one_fold(
    ...
    trn_loader, val_loader = get_data_loader(
        train_df, fold_id, classes=classes, cfg=CFG, pseudo_pool=pseudo_pool,
    )
...
def main(fold_ids: tp.Optional[tp.List[int]] = None,
    ...
    # ---- 4b) 加载伪标签池 (启用同类替换时) ----
    pseudo_pool: tp.Optional[PseudoPool] = None
    if getattr(CFG, "use_pseudo_replace", False):
        try:
            pseudo_pool = load_pseudo_pool(
                csv_path=PSEUDO_CSV,
                classes=classes,
                sc_dir=PSEUDO_SC_DIR,
            )
            print(
                f"[pseudo] enabled  prob={CFG.pseudo_replace_prob}  "
                f"covered_classes={len(pseudo_pool.pseudo_birds)}/{N_CLASSES}  "
                f"csv={PSEUDO_CSV}"
            )
        except FileNotFoundError as e:
            print(f"[pseudo] csv not found, fallback to no replacement: {e}")
            pseudo_pool = None
    else:
        print("[pseudo] disabled (CFG.use_pseudo_replace=False)")

    fold_ids = fold_ids if fold_ids is not None else list(range(N_FOLDS))
    for fold_id in fold_ids:
        train_one_fold(
            train_df, fold_id, device,
            classes=classes,
            output_dir=out_dir,
