...
# =============================================================================
# 4b. PseudoDS — 伪标签 soundscape 段 (软标签)
# =============================================================================

class PseudoDS(Dataset):
    """
    伪标签 soundscape 段 (来自 pseudo_runner 的 balanced csv).

    - 标签是 [0, 1] 软概率 (NUM_CLASSES 维), <0.1 已 trim 为 0.
    - 直接从 train_soundscapes/<audio_id>.ogg 用 soundfile 读 5s 段 (不走 waveform cache).
    - 每个 sample 返回 (waveform, label, weight, mask, source_tag="pseudo"|"pseudo_missing", perch_emb).
    - perch_emb 暂返回零向量, 让蒸馏 loss 在伪标签段上等价于 0
      (USE_PERCH_CACHE=False 时, train.py 会在线跑 ONNX teacher 给伪标签段算真 emb).

    Args:
        df:           pd.DataFrame, 必须含列:
                        row_id, primary_label, primary_label_prob, <NUM_CLASSES 个类列>
        classes:      长度 NUM_CLASSES 的类名列表 (跟 LABEL2IDX 对齐).
        sc_dir:       soundscape ogg 文件根目录 (PSEUDO_SC_DIR).
        aug:          是否开训练增强 (apply_aug + 训练随机起点).
    """

    def __init__(self, df, classes, sc_dir, aug=False):
        self.df       = df.reset_index(drop=True)
        self.classes  = list(classes)
        self.sc_dir   = Path(sc_dir)
        self.aug      = aug
        # 预解析 row_id -> (audio_id, end_sec) 加速 __getitem__
        self._sample_ids = np.empty(len(self.df), dtype=object)
        self._end_secs   = np.zeros(len(self.df), dtype=np.int32)
        for i, rid in enumerate(self.df["row_id"].values):
            parts = str(rid).rsplit("_", 1)
            self._sample_ids[i] = parts[0]
            self._end_secs[i]   = int(parts[1])
        # 预提取软标签矩阵 (n, NUM_CLASSES) float32
        self._soft = self.df[self.classes].values.astype(np.float32)

    def __len__(self):
        return len(self.df)

    def _load_segment(self, audio_id, end_sec):
        """从 sc_dir/<audio_id>.ogg 读取 [end_sec-5, end_sec) 那 5s. 失败返回 None."""
        path = self.sc_dir / f"{audio_id}.ogg"
        if not path.exists():
            return None
        start_sec = max(0, int(end_sec) - 5)
        try:
            with soundfile.SoundFile(str(path)) as f:
                sr = f.samplerate
                if sr != SR:
                    # train_soundscapes 应该都是 32k, 不一致就放弃这条
                    return None
                start = start_sec * sr
                f.seek(start)
                wav = f.read(frames=TRAIN_SAMPLES, dtype="float32")
                if wav.ndim > 1:
                    wav = wav.mean(axis=1)
                if len(wav) < TRAIN_SAMPLES:
                    wav = np.pad(wav, (TRAIN_SAMPLES - len(wav), 0))
                return wav
        except Exception:
            return None

    def __getitem__(self, i):
        wav = self._load_segment(self._sample_ids[i], self._end_secs[i])
        if wav is None:
            # 文件缺失 → source_tag=pseudo_missing, loss 权重 0
            return (torch.zeros(1, TRAIN_SAMPLES),
                    torch.zeros(NUM_CLASSES),
                    torch.ones(NUM_CLASSES),
                    torch.ones(NUM_CLASSES),
        ...
        return (torch.from_numpy(wav.astype(np.float32)).unsqueeze(0),
                torch.from_numpy(self._soft[i]),                # 软标签 [0,1]
                torch.ones(NUM_CLASSES),
                torch.ones(NUM_CLASSES),
                "pseudo",
                torch.zeros(PERCH_EMBED_DIM))                   # 伪标签段不预存 perch emb
