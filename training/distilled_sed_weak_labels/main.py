...
def _parse_args():
    ...
    p.add_argument("--lr", type=float, default=None,
                   help="override LR")
    p.add_argument("--amp", choices=["fp16", "bf16", "fp32"], default=None,
                   help="AMP dtype; convnextv2 建议 bf16")
    p.add_argument("--drop-path", type=float, default=None,
                   help="stochastic depth rate; 大模型 (hgnetv2_b3) 建议 0.2")
    p.add_argument("--workers", type=int, default=None,
                   help="DataLoader workers; 多终端并行时各 run 总和 ≤ CPU 核数")
...
def _apply_cli_overrides(args):
    ...
    if args.amp:        os.environ["BIRDCLEF_AMP"]            = args.amp
    if args.drop_path is not None: os.environ["BIRDCLEF_DROP_PATH"] = str(args.drop_path)
    if args.workers is not None: os.environ["BIRDCLEF_WORKERS"]     = str(args.workers)
