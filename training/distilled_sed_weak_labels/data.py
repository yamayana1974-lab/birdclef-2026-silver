...
from config import (COMP_DIR, WAVEFORM_CACHE_DIR, LABELS_PATH,
                    ...
                    USE_PSEUDO, PSEUDO_CSV, PSEUDO_SC_DIR)
...
def load_data():
    ...
    # ── 11. 伪标签 csv 加载 (★ 给 PseudoDS 用) ───────────────────
    # 读 pseudo_balanced.csv, 按训练 fold 划分:
    #   - 跟 sc 同 audio_id 的伪标签段, 继承该 sc 文件的 fold (防 val 泄漏)
    #   - 其他伪标签 audio_id (大部分): GroupKFold by audio_id 重新打 5 折
    pseudo_df = None
    if USE_PSEUDO:
        if not PSEUDO_CSV.exists():
            print(f"[pseudo] WARN csv not found: {PSEUDO_CSV}, skip pseudo source")
        else:
            ps = pd.read_csv(PSEUDO_CSV, low_memory=False)
            # 必要列校验
            need = {"row_id", "primary_label", "primary_label_prob"}
            miss = need - set(ps.columns)
            if miss:
                print(f"[pseudo] WARN missing cols {miss}, skip pseudo source")
            else:
                # 检查 234 类列齐全 (跟 PRIMARY_LABELS 顺序对齐)
                miss_cls = [c for c in PRIMARY_LABELS if c not in ps.columns]
                if miss_cls:
                    print(f"[pseudo] WARN missing class cols (前5): {miss_cls[:5]}, "
                          f"skip pseudo source")
                else:
                    # row_id = "<audio_id>_<end_sec>" 还原 audio_id
                    ps["audio_id"] = ps["row_id"].apply(
                        lambda x: "_".join(str(x).split("_")[:-1]))

                    # fold 划分: 跟 sc 共享 audio_id 的继承, 其他 GroupKFold
                    ps_aids = ps["audio_id"].unique().tolist()
                    sc_aids = set(sc_cache_meta["filename"].apply(
                        lambda x: x.replace(".ogg", "")).tolist())
                    aid2fold = {}
                    # 优先继承 sc 那边的 fold
                    sc_aid_to_fold = {}
                    for _, r in sc_cache_meta.iterrows():
                        aid = r["filename"].replace(".ogg", "")
                        sc_aid_to_fold[aid] = int(r["fold"])
                    leftover = []
                    for a in ps_aids:
                        if a in sc_aid_to_fold:
                            aid2fold[a] = sc_aid_to_fold[a]
                        else:
                            leftover.append(a)
                    # 剩下的随机打 5 折 (round-robin)
                    rng_ps = np.random.RandomState(SEED)
                    rng_ps.shuffle(leftover)
                    for k, a in enumerate(leftover):
                        aid2fold[a] = k % N_FOLDS
                    ps["fold"] = ps["audio_id"].map(aid2fold).astype(int)

                    pseudo_df = ps.reset_index(drop=True)
                    print(f"\n[pseudo] loaded {len(pseudo_df)} segments from "
                          f"{pseudo_df['audio_id'].nunique()} audios "
                          f"({len(leftover)} new + {len(ps_aids) - len(leftover)} "
                          f"shared with sc)")
                    print(f"[pseudo] fold dist: "
                          f"{pseudo_df['fold'].value_counts().sort_index().to_dict()}")
                    print(f"[pseudo] prob: min={pseudo_df['primary_label_prob'].min():.3f} "
                          f"median={pseudo_df['primary_label_prob'].median():.3f} "
                          f"max={pseudo_df['primary_label_prob'].max():.3f}")

    # ── 12. DEBUG 子采样 (Kaggle 上调试用) ──────────────────────
    if DEBUG:
        ...
        if pseudo_df is not None:
            pseudo_df = pseudo_df.head(50).reset_index(drop=True)
        print(f"DEBUG MODE: {len(audio_cache_meta)} focal, "
              f"{len(sc_cache_meta)} sc"
              + (f", {len(pseudo_df)} pseudo" if pseudo_df is not None else ""))
    ...
    return {
        ...
        "pseudo_df":              pseudo_df,                 # ★ 新增, None=未启用
    }
