...
> **路径默认是 `./data` 和 `./models`** (相对仓库根), 可用文档里的 `BIRDCLEF_*`
> 环境变量覆盖 (如 `BIRDCLEF_BASE`、`BIRDCLEF_TRAIN_ROOT`、`BIRDCLEF_SED_ROOT`),
> 不要直接改代码。在 Kaggle 上流水线会自动探测 `/kaggle/input/...`。各模块 README
> 列出了相关变量。
