...
#
#   ── EfficientNetV2 (b0~b3 仅 TF 移植版有 in1k 权重, 必须带 tf_ 前缀) ──
#   "tf_efficientnetv2_b0.in1k"           7.1M  out C=1280
#   "tf_efficientnetv2_b2.in1k"          10.1M  out C=1408  ← ★ 已验证可跑
#   "tf_efficientnetv2_b3.in1k"          14.4M  out C=1536
#
#   ── HGNetV2 (PaddlePaddle ssld 两阶段半监督蒸馏权重) ──
#   "hgnetv2_b0.ssld_stage2_ft_in1k"      4.0M  out C=2048  ← HGNet 同量级对比
#   "hgnetv2_b3.ssld_stage2_ft_in1k"     14.2M  out C=2048  ← ★ 已验证可跑
#   "hgnetv2_b5.ssld_stage2_ft_in1k"     37.5M  out C=2048
#
#   ── ConvNeXtV2 (FCMAE 自监督 + in1k 微调; 用 ft_in1k, 不要光 .fcmae 那个无分类头) ──
#   "convnextv2_atto.fcmae_ft_in1k"       3.4M  out C=320
#   "convnextv2_pico.fcmae_ft_in1k"       8.6M  out C=512   ← ★ 已验证可跑 (下采样 32×, 时间维偏粗)
#   "convnextv2_nano.fcmae_ft_in22k_in1k" 15.0M out C=640
#
#   ── 其他 hybrid (供参考) ──
...
#
# ★ 注意: backbone_dim (out C) 由 models.py 跑 dummy forward 自动探测, 上面的 C 仅供参考.
#         换 backbone 后 export_onnx.py 也会自适应, 无需手改其他地方.
# ★ 显存: b3 / v2_b2 (10~14M) 比默认 B0 大, BATCH=64 可能 OOM, 不够就 --batch 32.
#
...
# Stochastic depth (drop_path) 比率, 传给 timm backbone.
#   小模型 (B0) 0.1 够; 大模型 (hgnetv2_b3 等) 0.2 更抗过拟合.
#   优先级: CLI --drop-path > 环境变量 BIRDCLEF_DROP_PATH > 默认 0.1
DROP_PATH_RATE = float(os.environ.get("BIRDCLEF_DROP_PATH", 0.1))
...
#   设 0 = 关闭早停, 跑满 EPOCHS.
EARLY_STOP_PATIENCE = 0                           # 0 关闭; 8 = 连续 8 epoch 没涨就停

# DataLoader (RTX PRO 6000 + NVMe 配置)
#   ★ 多终端并行时 NUM_WORKERS 要降: 各 run 的 workers 总和 ≤ CPU 核数 (nproc),
#     否则 worker 进程互相抢 CPU 反而变慢. 例如 16 核跑 3 个 run, 每个设 5.
#   优先级: 环境变量 BIRDCLEF_WORKERS > 默认 16
NUM_WORKERS        = int(os.environ.get("BIRDCLEF_WORKERS", 16))  # 0 = 单线程 (Kaggle)
PIN_MEMORY         = True
PERSISTENT_WORKERS = True                        # epoch 间 worker 不重启

# AMP 精度 (优先级: CLI --amp > 环境变量 BIRDCLEF_AMP > 默认 fp16)
#   "fp16" = 跟 notebook 一致 (用 GradScaler, T4/P100 也支持)
#   "bf16" = Blackwell/H100 数值稳定, 不需要 GradScaler (速度差不多, 训练更稳)
#   ★ convnextv2 建议 bf16: ConvNeXt 的 LayerNorm/GRN 在 fp16 下偶发 NaN/不收敛.
AMP_DTYPE          = os.environ.get("BIRDCLEF_AMP", "fp16")   # "bf16" | "fp16" | "fp32"
...
# =============================================================================
# 13. 多源 batch 组成 (focal + labeled soundscape + pseudo)
# =============================================================================
...
# ── 伪标签数据源 (来自 pseudo_runner 的 balanced csv) ──
# 思路: train_soundscapes/ 中未在 train_soundscapes_labels.csv 出现的 ~5938 个文件,
# 用三套模型 (v6/v13/v17) ensemble 软标签, 经过 per-class 阈值 + capping 后得到
# pseudo_balanced.csv (33699 段). 把这些段当作"软标签 sc" 加入训练, sc 域数据从
# 1% 拉到 ~30%, 大幅缩小训练 / 测试域漂移.
USE_PSEUDO          = bool(int(os.environ.get("BIRDCLEF_USE_PSEUDO", "1")))
# 伪标签 csv 路径. 跟 hgnet_train_test 共用同一份 pseudo_balanced.csv.
PSEUDO_CSV          = Path(os.environ.get(
    "BIRDCLEF_PSEUDO_CSV",
    str(_PROJECT_ROOT / "pseudo_output" / "pseudo_balanced.csv"),
))
# 伪标签段对应的 soundscape ogg 目录 (跟 train_soundscapes 同源).
PSEUDO_SC_DIR       = COMP_DIR / "train_soundscapes"

# Batch 内组成比例
#   开 USE_PSEUDO: focal 70% + sc 10% + pseudo 20%
#   关 USE_PSEUDO: focal 90% + sc 10% (退化为原行为)
if USE_PSEUDO:
    ACTIVE_SOURCES = ["focal", "sc", "pseudo"]
    SHARES = {"focal": 0.7, "sc": 0.1, "pseudo": 0.2}
else:
    ACTIVE_SOURCES = ["focal", "sc"]
    SHARES = {"focal": 0.9, "sc": 0.1}

# 不同数据源的 sample-level loss 权重
# pseudo 设 0.5, 跟 hgnet_train_test 的 pseudo_loss_weight 对齐, 让真实数据主导梯度.
SOURCE_WEIGHTS = {
    "focal":         1.0,
    "focal_missing": 0.0,                         # cache 缺失文件 → 不算 loss
    "sc":            1.0,
    "pseudo":        float(os.environ.get("BIRDCLEF_PSEUDO_W", "0.5")),
    "pseudo_missing": 0.0,                        # 伪标签 ogg 缺失 → 不算 loss
}
